...
const createBook = async function (req, res) {
  try {
    ...
    if (!isEmpty(userId)) {
      return res
        .status(400)
        .send({ status: false, msg: "Please provide user ID" });
    }
    if (!isValidObjectId(userId)) {
      return res
        .status(400)
        .send({ status: false, msg: "Provide a valid user id" });
    }
    ...
    if (!isEmpty(excerpt)) {
      return res
        .status(400)
        .send({ status: false, msg: "Please provide excerpt of blog" });
    }
    if (!isEmpty(category)) {
      return res
        .status(400)
    ...
    if (!isValidISBN(ISBN)) {
      return res
        .status(400)
        .send({ status: false, msg: "Please provide a valid ISBN" });
    }
    let checkISBN = await bookModel.findOne({ ISBN: ISBN });
    if (checkISBN)
      return res
        .status(404)
        .send({ status: false, message: " ISBN must be unique " });

    if (!releasedAt) {
      return res
        .status(400)
        .send({ status: false, message: "Must present releseAt" });
    }
    if (!moment.utc(releasedAt, "YYYY-MM-DD", true).isValid())
      return res
        ...
        .send({
          status: false,
          message: "enter date in valid format eg. (YYYY-MM-DD)...!",
        });

    let bookData = await bookModel.create(data);
    res
      .status(201)
      .send({ status: true, msg: "Blog has been created", data: bookData });
  } catch (err) {
...
const getBooks = async (req, res) => {
  try {
    ...
    if (!data)
      return res
        .status(404)
        .send({ status: false, msg: "No data found in query" });
    ...
    if (getBook.length == 0)
      return res
        .status(404)
        .send({ status: false, message: "no documents found with this query" });
...
const bookDetails = async function (req, res) {
  try {
    ...
    if (getBook.length == 0)
      return res
        .status(404)
        .send({ status: false, msg: "no documents found " });
  ...
  try {
    ...
    if (Object.keys(data).length == 0) {
      return res
        ...
        .send({
          status: false,
          message: "Please enter book details for updating",
        });
    }
    if (excerpt) {
      if (!isEmpty(excerpt))
    ...
    if (checkISBN) {
      return res
        .status(400)
        .send({ status: false, message: "Book already exist with this ISBN" });
    }

    let updatedBooks = await bookModel.findOneAndUpdate(
      { _id: bookId },
      { $set: { title: title, excerpt: excerpt, ISBN } },
      { new: true }
    );
    return res
      .status(200)
      .send({ status: true, message: "Update successful", data: updatedBooks });
  } catch (err) {
  ...
  try {
    ...
    if (checkBook.isDeleted == false) {
      ...
      return res
        ...
        .send({
          status: true,
          message: "book is deleted sussesfully",
          data: deletedDoc,
        });
    }
  } catch (err) {
