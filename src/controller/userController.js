...
const createUser = async function (req, res) {
  try {
    ...
    if (!isEmpty(name)) {
      return res
        .status(400)
        .send({ status: false, msg: "Name must be present " });
    }
    ...
    if (!isValidPhone(phone))
      return res
        .status(400)
        .send({ status: false, message: "Please Enter 10 digit phone Number" });
    ...
    if (!street(address.street)) {
      return res
        .status(400)
        .send({ status: false, msg: "Enter valid Street Name" });
    }

    if (!city(address.city)) {
      return res
        .status(400)
        .send({ status: false, msg: "Enter valid City Name" });
    }

    if (!pincode(address.pincode)) {
      return res
        .status(400)
        .send({ status: false, msg: "Enter valid Pin code" });
    }

    const result = await userModel.create(data);
    res
      .status(201)
      .send({ status: true, msg: "new user is created", data: result });
  } catch (err) {
